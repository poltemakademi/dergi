...
import { Fingerprint, X, Info } from 'lucide-react';
        ...
        <div className="max-w-[1200px] mx-auto relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-16">
            ...
            <div className="md:col-span-4 lg:col-span-5">
              ...
              <div className="flex gap-4 mt-8">
                <a href="#" onClick={(e) => e.preventDefault()} className="w-10 h-10 rounded-full bg-slate-900 border border-slate-800 flex items-center justify-center text-slate-400 hover:text-white hover:bg-indigo-600 hover:border-indigo-500 hover:-translate-y-1 transition-all duration-300 shadow-sm hover:shadow-[0_0_15px_rgba(79,70,229,0.4)]">
            ...
            <div className="md:col-span-8 lg:col-span-7 grid grid-cols-2 sm:grid-cols-3 gap-8">
              <div className="flex flex-col gap-4 items-start">
                ...
                <Link to="/sistem-ozellikleri" className="text-slate-400 hover:text-indigo-400 font-medium transition-colors inline-flex items-center gap-1.5 group">
                  <span className="w-0 group-hover:w-2 h-0.5 bg-indigo-500 transition-all duration-300"></span> {t.footer.sys}
                </Link>
                <Link to="/entegrasyonlar" className="text-slate-400 hover:text-indigo-400 font-medium transition-colors inline-flex items-center gap-1.5 group">
                  <span className="w-0 group-hover:w-2 h-0.5 bg-indigo-500 transition-all duration-300"></span> {t.footer.int}
                </Link>
                <Link to="/early-access" className="text-slate-400 hover:text-indigo-400 font-medium transition-colors inline-flex items-center gap-1.5 group">
                  <span className="w-0 group-hover:w-2 h-0.5 bg-indigo-500 transition-all duration-300"></span> {t.footer.early}
                </Link>
                <button onClick={(e) => openModal(e, t.footer.contact, 'contact')} className="text-slate-400 hover:text-indigo-400 font-medium transition-colors inline-flex items-center gap-1.5 group mt-2">
                  <span className="w-0 group-hover:w-2 h-0.5 bg-indigo-500 transition-all duration-300"></span> {t.footer.contact}
                </button>
              </div>

              <div className="flex flex-col gap-4 items-start">
                <h4 className="text-slate-50 font-bold tracking-wider text-sm uppercase mb-2">{t.footer.res}</h4>
                <button onClick={(e) => openModal(e, t.footer.apply, 'apply')} className="text-slate-400 hover:text-indigo-400 font-medium transition-colors inline-flex items-center gap-1.5 group">
                  <span className="w-0 group-hover:w-2 h-0.5 bg-indigo-500 transition-all duration-300"></span> {t.footer.apply}
                </button>
                <button onClick={(e) => openModal(e, t.footer.docs, 'docs')} className="text-slate-400 hover:text-indigo-400 font-medium transition-colors inline-flex items-center gap-1.5 group">
                  <span className="w-0 group-hover:w-2 h-0.5 bg-indigo-500 transition-all duration-300"></span> {t.footer.docs}
                </button>
                <button onClick={(e) => openModal(e, t.footer.api, 'api')} className="text-slate-400 hover:text-indigo-400 font-medium transition-colors inline-flex items-center gap-1.5 group">
                  <span className="w-0 group-hover:w-2 h-0.5 bg-indigo-500 transition-all duration-300"></span> {t.footer.api}
                </button>
              </div>

              <div className="flex flex-col gap-4 items-start">
                <h4 className="text-slate-50 font-bold tracking-wider text-sm uppercase mb-2">{t.footer.legal}</h4>
                <button onClick={(e) => openModal(e, t.footer.ethics, 'ethics')} className="text-slate-400 hover:text-indigo-400 font-medium transition-colors inline-flex items-center gap-1.5 group">
                  <span className="w-0 group-hover:w-2 h-0.5 bg-indigo-500 transition-all duration-300"></span> {t.footer.ethics}
                </button>
                <button onClick={(e) => openModal(e, t.footer.openAccess, 'openAccess')} className="text-slate-400 hover:text-indigo-400 font-medium transition-colors inline-flex items-center gap-1.5 group">
                  <span className="w-0 group-hover:w-2 h-0.5 bg-indigo-500 transition-all duration-300"></span> {t.footer.openAccess}
                </button>
                <button onClick={(e) => openModal(e, t.footer.kvkk, 'kvkk')} className="text-slate-400 hover:text-indigo-400 font-medium transition-colors inline-flex items-center gap-1.5 group">
                  <span className="w-0 group-hover:w-2 h-0.5 bg-indigo-500 transition-all duration-300"></span> {t.footer.kvkk}
                </button>
                <button onClick={(e) => openModal(e, t.footer.sales, 'sales')} className="text-slate-400 hover:text-indigo-400 font-medium transition-colors inline-flex items-center gap-1.5 group">
                  <span className="w-0 group-hover:w-2 h-0.5 bg-indigo-500 transition-all duration-300"></span> {t.footer.sales}
                </button>
              </div>
            </div>
          ...
          <div className="border-t border-slate-800/80 pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="text-sm font-medium text-slate-500 flex items-center gap-2">
              ...
              &copy; {new Date().getFullYear()} novaijournal. {t.footer.rights}
            </div>
            <div className="flex items-center gap-6 text-sm font-medium text-slate-500">
              <button onClick={(e) => openModal(e, t.footer.terms, 'terms')} className="hover:text-slate-300 transition-colors">{t.footer.terms}</button>
              <button onClick={(e) => openModal(e, t.footer.privacy, 'privacy')} className="hover:text-slate-300 transition-colors">{t.footer.privacy}</button>
              <button onClick={(e) => openModal(e, t.footer.cookies, 'cookies')} className="hover:text-slate-300 transition-colors">{t.footer.cookies}</button>
            </div>
          </div>
        </div>
      </footer>
