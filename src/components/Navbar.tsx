...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed z-50 transition-all duration-500 left-1/2 -translate-x-1/2 max-w-[1200px] w-[calc(100%-2rem)] flex flex-col justify-between ${scrolled ? 'top-4 bg-white/90 backdrop-blur-2xl border border-slate-200/50 shadow-[0_8px_30px_rgb(0,0,0,0.06)] py-2.5 px-4 rounded-[2rem]' : 'top-6 bg-white border border-slate-200/80 shadow-[0_4px_20px_rgb(0,0,0,0.03)] py-3 px-5 rounded-[2.2rem]'}`}>
        ...
        <div className="flex items-center gap-2.5 pr-1">
          ...
          {isAuthenticated ? (
            ...
            <>
              ...
              <Link to="/auth" className="relative group overflow-hidden px-6 py-2.5 bg-slate-900 text-white text-xs font-bold uppercase tracking-wider rounded-full shadow-[0_8px_20px_-6px_rgba(0,0,0,0.3)] hover:shadow-[0_8px_25px_-5px_rgba(79,70,229,0.4)] transition-all duration-300 cursor-pointer flex items-center gap-2">
                <div className="absolute inset-0 bg-gradient-to-r from-indigo-600 to-sky-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      ...
      <AnimatePresence>
        {isJournalPage && scrolled && (
          <motion.div
            ...
            className="w-full border-t border-slate-100/80 pt-2 flex items-center justify-center gap-5 overflow-x-auto scrollbar-none"
          >
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => handleTabClick(tab.id)}
                className={`relative pb-2 pt-1 px-1 text-[11px] font-black uppercase tracking-wider transition-all cursor-pointer whitespace-nowrap ${activeTab === tab.id
                  ? 'text-indigo-600'
